...
set cppfilePath=D:\\Project\\SWTest\\CppCLR_WinformsProjekt1\\CppCLR_WinformsProjekt1\\SwComm.h
set enumNam=EUiMark
...
for /F %%i in (tmp.data) do (
	if not "%%i" == "output.txt" if not "%%i" == "tmp.data" (
		@rem echo %strHead%%%i"
		set strResult=%%i"
		call:GetLength %%i
